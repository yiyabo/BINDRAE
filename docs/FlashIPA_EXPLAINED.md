...
| 指标 | 值 | 说明 |
|-----|---|------|
| **χ1准确率** | 71.1% | 早期 Stage-1 配置，> 初始目标70% ⭐ |
| **FAPE** | 0.05 Å | 优秀 |
| **显存占用** | 54 MB/样本 | vs 传统~150 MB |
| **训练速度** | 8秒/epoch | 稳定 |
| **参数量** | 10M（IPA部分） | 11.4M总计 |

> 注：后续在 Stage-1 中加入多层 ligand conditioning、χ1 rotamer 头、20D ligand 特征后，
> 整体 CASF-2016 val χ1 准确率进一步提升到 **≈75–76%**。FlashIPA 本身的配置（例如 `z_rank=2`）保持不变，
> 说明在最终架构下 FlashIPA 仍然是足够的几何主干。

### 结论

**FlashIPA (z_rank=2) 在本项目完全可行！**
- 性能达标（无论是早期 71% 还是最终 75–76% 的 Stage-1 配置）
- 显存友好
- 训练稳定
