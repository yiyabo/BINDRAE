...
```
# 早期 Stage-1 配置的性能（12D ligand 类型特征）
χ1准确率: 71.1% > 初始目标70% ✅
Val Loss: 0.21（泛化良好）✅
...
> 注：在最终 Stage-1 架构中（加入多层 ligand conditioning、χ1 rotamer 头、20D ligand 类型/拓扑特征后），
> CASF-2016 val χ1 准确率进一步提升到 **≈75–76%**。方向探针设计本身保持不变，
> 说明它不仅在早期配置下有效，也支持最终版本的 Stage-1。
...
```python
{
    ...
    'types': [M, 20],      # 20维类型/拓扑特征（由原先的12维类型编码扩展而来）
    'is_probe': [M],       # 是否是探针
