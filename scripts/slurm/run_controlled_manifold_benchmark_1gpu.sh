#!/bin/bash
# Controlled SE(3) x torus phase/normal identifiability benchmark.

#SBATCH --job-name=s2_ctrl_manifold
...
#SBATCH --output=logs/slurm/stage2_controlled_manifold_%j.out
#SBATCH --error=logs/slurm/stage2_controlled_manifold_%j.err
...
TAG="${TAG:-controlled_manifold_$(date +%Y%m%d_%H%M%S)}"
