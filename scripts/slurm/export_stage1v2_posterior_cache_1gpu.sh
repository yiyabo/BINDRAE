...
#SBATCH --output=logs/slurm/stage1v2_cache_%j.out
#SBATCH --error=logs/slurm/stage1v2_cache_%j.err
...
cd "$ROOT"
mkdir -p logs/slurm logs/stage1v2_student_posteriors
...
LABEL_DIR="${LABEL_DIR:-logs/stage1v2_teacher_posteriors/holo_truth_train4096_lcpgbf_20260622}"
SPLIT="${SPLIT:-train}"
VALID_SAMPLES_FILE="${VALID_SAMPLES_FILE:-}"
TAG="${TAG:-stage1v2_student_train4096_lcpgbf_best_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-logs/stage1v2_student_posteriors/${TAG}}"
...
if [[ "${INCLUDE_TEACHER_LABELS:-0}" == "1" ]]; then
  ARGS+=(--include_teacher_labels)
fi
if [[ "${NO_AMP:-0}" == "1" ]]; then
  ARGS+=(--no_amp)
fi

echo "Stage-1-v2 posterior cache export"
echo "  checkpoint:             $CHECKPOINT"
echo "  split:                  $SPLIT"
echo "  label_dir:              $LABEL_DIR"
echo "  output_dir:             $OUTPUT_DIR"
echo "  include_teacher_labels: ${INCLUDE_TEACHER_LABELS:-0}"
echo "  started:                $(date)"
