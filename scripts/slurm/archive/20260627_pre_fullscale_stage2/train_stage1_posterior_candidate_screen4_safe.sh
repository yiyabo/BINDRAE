#!/bin/bash
#SBATCH --job-name=s1_postcand4safe
...
#SBATCH --output=logs/slurm/stage1_posterior_candidate_screen4_safe_%j.out
#SBATCH --error=logs/slurm/stage1_posterior_candidate_screen4_safe_%j.err
...
TAG="${TAG:-posterior_candidate_ligcausal_screen4safe_$(date +%Y%m%d_%H%M%S)}"
...
LR="${LR:-2e-5}"
# Branch-resume source checkpoint is at epoch 22; 26 gives ~4 additional epochs.
MAX_EPOCHS="${MAX_EPOCHS:-26}"
LAMBDA_CANDIDATE_CHI1="${LAMBDA_CANDIDATE_CHI1:-0.4}"
LAMBDA_LIGAND_CONTRASTIVE="${LAMBDA_LIGAND_CONTRASTIVE:-0.2}"
LIGAND_CONTRASTIVE_MARGIN="${LIGAND_CONTRASTIVE_MARGIN:-0.4}"
CANDIDATE_SCORER_LR_SCALE="${CANDIDATE_SCORER_LR_SCALE:-4.0}"
LAMBDA_CONTACT="${LAMBDA_CONTACT:-0.1}"
MASTER_PORT="${MASTER_PORT:-29521}"

cat <<EOF
============================================================
Stage-1 candidate/contact posterior screen SAFE retry (4-GPU)
============================================================
...
Max epochs:       $MAX_EPOCHS (absolute branch-resume epoch limit)
...
Why this retry:
  8-GPU run 128765 reached epoch 22 ~60% but failed with NCCL ALLREDUCE
  timeout before first validation. This retry reduces DDP width, lowers LR and
  contrastive pressure, and enables find_unused_parameters for frozen/posterior
  branch safety.
Start: $(date)
============================================================
EOF
...
echo ""
echo "============================================================"
echo "Posterior candidate safe retry completed: $(date)"
echo "Checkpoint: checkpoints/stage1/$TAG"
echo "Metrics:    logs/stage1/$TAG/metrics.jsonl"
echo "Next: run posterior diagnostics on best_model.pt"
echo "============================================================"
