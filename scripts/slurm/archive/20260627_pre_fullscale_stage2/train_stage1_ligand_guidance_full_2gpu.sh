...
#SBATCH --output=logs/slurm/stage1_ligand_guidance_full_2gpu_%j.out
#SBATCH --error=logs/slurm/stage1_ligand_guidance_full_2gpu_%j.err
...
TAG="${TAG:-ligand_guidance_full_2gpu_$(date +%Y%m%d_%H%M%S)}"
...
cat <<EOF
============================================================
Stage-1 ligand internal guidance FULL training (2-GPU, RAEv2-inspired)
============================================================
Job ID:          ${SLURM_JOB_ID:-NA}
Node:            ${SLURM_NODELIST:-NA}
GPUs:            2 x A100 on one node
Tag:             $TAG
Resume:          $RESUME_FROM
