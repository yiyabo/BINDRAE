...
#SBATCH --output=logs/slurm/stage1_contrastive_only_1gpu_%j.out
#SBATCH --error=logs/slurm/stage1_contrastive_only_1gpu_%j.err
...
TAG="${TAG:-contrastive_only_smoke_1gpu_$(date +%Y%m%d_%H%M%S)}"
...
LR="${LR:-3e-4}"
LAMBDA_DECOY_CONTRASTIVE="${LAMBDA_DECOY_CONTRASTIVE:-5.0}"
...
cat <<EOF
============================================================
Stage-1 contrastive-only smoke (no chi1 loss)
============================================================
...
LR:              $LR
Lambda chi1:     0.0 (DISABLED)
Lambda contrastive: $LAMBDA_DECOY_CONTRASTIVE
Selection metric:$SELECTION_METRIC
Batch size:      $BATCH_SIZE
Residue budget:  $RESIDUE_BUDGET
Goal:            test contrastive-only training to force ligand discrimination
Start:           $(date)
============================================================
EOF
...
python scripts/train_stage1.py \
  ...
  --lambda_decoy_contrastive "$LAMBDA_DECOY_CONTRASTIVE" \
  --decoy_contrastive_margin 0.1 \
  --decoy_contrastive_repulsion 0.1 \
  --decoy_contrastive_decoy_kind scrambled \
  --lambda_typed_candidate_energy 0.01 \
  --typed_candidate_decoy_kind scrambled \
...
echo ""
echo "============================================================"
echo "Contrastive-only smoke completed: $(date)"
