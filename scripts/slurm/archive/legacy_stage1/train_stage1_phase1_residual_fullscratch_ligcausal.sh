#!/bin/bash
#SBATCH --job-name=s1_p1_zero_lift
...
#SBATCH --output=logs/slurm/stage1_phase1_fullscratch_ligcausal_%j.out
#SBATCH --error=logs/slurm/stage1_phase1_fullscratch_ligcausal_%j.err
...
TAG="${TAG:-phase1_residual_fullscratch_ligcausal_$(date +%Y%m%d_%H%M%S)}"
...
cat <<EOF
==============================================
Stage-1 Phase-1 Residual: full from-scratch ligand-causal training (4-GPU)
==============================================
...
Scientific target:
  Train the complete Stage-1 model from scratch using the phase1-residual
  base+ligand-residual geometry scorer, then select checkpoints by ligand-causal
  contact/switch lift over decoy ligand rather than by global chi1 accuracy.

Controls:
  resume checkpoint:     NONE (true from-scratch run)
  freeze base_mlp:       NO  (learn base prior in the full model)
  freeze trunk/backbone: NO  (full Stage-1 training, not posterior-only)
...
echo ""
echo "=============================================="
echo "Full from-scratch ligand-causal Phase-1 residual training completed: $(date)"
