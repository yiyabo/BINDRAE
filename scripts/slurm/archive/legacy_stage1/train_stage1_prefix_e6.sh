#!/bin/bash
# Shared-prefix screening template for E6-style experiments.
# Current recommendation: stop at epoch_003.pt so branches resume before pchi1_start_step=8000.

#SBATCH --job-name=s1_prefix_e6
...
#SBATCH --output=logs/slurm/stage1_prefix_e6_%j.out
#SBATCH --error=logs/slurm/stage1_prefix_e6_%j.err
...
PREFIX_TAG="prefix_e6_$(date +%Y%m%d_%H%M%S)"

echo "=============================================="
echo "Stage-1 shared-prefix E6 screening run (pre-pchi1)"
echo "=============================================="
echo "Job ID:      $SLURM_JOB_ID"
echo "Checkpoint:  checkpoints/stage1/$PREFIX_TAG"
echo "Log dir:     logs/stage1/$PREFIX_TAG"
echo "=============================================="
...
torchrun \
  ...
  --save_dir checkpoints/stage1/${PREFIX_TAG} \
  --log_dir logs/stage1/${PREFIX_TAG} \
  --distributed
