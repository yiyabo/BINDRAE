...
#SBATCH --mem=48G
#SBATCH --output=logs/slurm/stage1_candidate_smoke_%j.out
#SBATCH --error=logs/slurm/stage1_candidate_smoke_%j.err
...
TAG="${TAG:-candidate_chi1_lcon_single_smoke_$(date +%Y%m%d_%H%M%S)}"
...
LR="${LR:-3e-5}"
MAX_EPOCHS="${MAX_EPOCHS:-22}"
MAX_N_RES="${MAX_N_RES:-900}"
RESIDUE_BUDGET="${RESIDUE_BUDGET:-900}"
LAMBDA_CANDIDATE_CHI1="${LAMBDA_CANDIDATE_CHI1:-0.1}"
LAMBDA_LIGAND_CONTRASTIVE="${LAMBDA_LIGAND_CONTRASTIVE:-0.05}"

echo "=============================================="
echo "Stage-1 candidate-aware single-GPU smoke"
echo "=============================================="
echo "Job ID:     $SLURM_JOB_ID"
echo "Node:       $SLURM_NODELIST"
echo "Tag:        $TAG"
echo "Resume:     $RESUME_FROM"
echo "Max n_res:  $MAX_N_RES"
echo "Resid budget: $RESIDUE_BUDGET"
...
echo ""
echo "=============================================="
echo "Candidate smoke completed: $(date)"
