#!/usr/bin/env bash
# Run adaptive CA-ANM projection baseline and evaluate ligand-pocket CA metrics.

#SBATCH --job-name=aanm_base
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=04:00:00
#SBATCH --output=logs/slurm/adaptive_anm_baseline_%j.out
#SBATCH --error=logs/slurm/adaptive_anm_baseline_%j.err
...
TAG="${TAG:-adaptive_anm20_clean309_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-reference/baselines/AdaptiveANM/runs/${TAG}}"
...
echo "=============================================="
echo "BINDRAE adaptive CA-ANM baseline"
echo "=============================================="
...
echo "Step mode:    ${STEP_MODE:-fraction}"
echo "Start:        $(date)"
echo "=============================================="

python scripts/run_adaptive_anm_baseline.py \
  ...
  --step_mode "${STEP_MODE:-fraction}" \
  --max_rescale "${MAX_RESCALE:-5.0}" \
  ${CLEAN:+--clean} \
...
python scripts/evaluate_ebdims2_ca_paths.py \
  ...
  --method_prefix "${METHOD_PREFIX:-adaptive_anm_ca}" \
  --frame_offset none
