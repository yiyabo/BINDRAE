...
我们研究在已知蛋白序列、apo 构象与配体结合姿态（bound pose）的条件下，生成从 apo 到 holo 的**连续构象通路**并预测最终结合构象。**当前主线聚焦 known pose 的 induced‑fit / 路径生成，不将 docking 作为主线贡献**；对 pose 误差的鲁棒性通过 pose 噪声敏感性曲线评估，docking 作为未来扩展。我们提出一个两阶段框架：
...
**Stage‑2** 在乘积流形 $\mathcal M=\mathrm{SE(3)}^N\times (S^1)^{4N}$ 上学习一个**配体条件化、口袋—变构联合门控**的桥流向量场，使用 deterministic 参考桥与 Conditional Flow Matching 训练，并将 Stage‑1 的几何先验（FK/FAPE/clash/contact）提升为**路径级约束**。为覆盖少数但关键的“大尺度变构”，Stage‑2 可选引入基于弹性网络模型（ENM）的 **NMA 残基层移动性特征**，以物理可解释方式指导门控与损失权重，实现对铰链/结构域运动的自适应激活。
...
* 任务定位：默认假设该结合姿态已知（known pose）；docking/pose 搜索不作为主线贡献，但需通过 pose 噪声敏感性曲线明确适用边界（见 §10）。
...
$$
其中 $\psi$ 为 Stage‑1 参数。Stage‑1 输出将作为 Stage‑2 的**先验终态锚**与条件输入（见 §5.1、§6.5）。
...
**方向探针（Direction Probes）**：默认将配体 token 扩展为“重原子 + 方向探针（虚拟点）”的集合，以显式提供 HBD/HBA/芳香/带电等相互作用的方向信息；探针与原子一并参与跨注意力/消息传递（实现细节见 `docs/LIGAND_PROBES_EXPLAINED.md`）。
...
**多层配体注入（实现约定）**：默认采用 layerwise conditioning——在 ESM Adapter 后先注入一次配体条件化信息，并在每层 FlashIPA/IPA 更新后再次注入（作为 ablation 可退化为 one‑shot 注入）。
...
### 2.5.5 训练 warmup（稳定性约定）

为避免早期训练被配体噪声与口袋权重过强放大，采用两个严格 warmup：

1. **Pocket warmup**：令 $\kappa(s)\in[0,1]$ 随训练步 $s$ 线性上升（到达 $s_{\mathrm{pocket}}$ 后取 1），并使用
$$
w_i^{\mathrm{pocket}}(s)=(1-\kappa(s))\,w_{\mathrm{base}}+\kappa(s)\,w_i^{\mathrm{pocket}},
\quad w_{\mathrm{base}}\approx 0.1.
$$
2. **Ligand gate warmup**：令 $\lambda(s)\in[0,1]$ 随训练步线性上升（到达 $s_{\mathrm{lig}}$ 后取 1），用于缩放配体条件化分支（cross‑attn/FiLM）对蛋白特征的影响强度，使模型先学到稳定的几何 prior，再逐步强化配体驱动。
...
2. **路径后半段先验约束**：对 $t$ 接近 1 的状态施加软约束（见 §6.5），避免桥流在终段偏离可行 holo 流形。
...
其中条件
$$
c=\{S,\ L,\ \mathbf X^{\mathrm{apo}},\ \tilde x_1,\ w_i^{\mathrm{pocket}},\ (M_i^{\mathrm{nma}})\}.
$$
其中括号项表示可选插件（默认关闭）。
...
Stage‑2 训练使用 deterministic 参考桥 $X_t^{\mathrm{ref}}$ 与其解析速度 $u_t^{\mathrm{ref}}$，并通过 Conditional Flow Matching 学习真实向量场。

---

## 4.1 χ（圆周空间）上的 deterministic 参考路径
...
3. 参考桥采样：
$$
   \chi_t^{\mathrm{ref}}=\mathrm{wrap}\bigl(\chi_0+\gamma(t)\Delta\chi\bigr).
$$
...
## 4.2 $\mathrm{SE(3)}$ 上的 geodesic 参考桥
...
消融设置下若不使用 Stage‑1 条件，则 $\log\delta F_i$ 与 $\delta\chi_i$ 置 0。
...
$$
若未启用 NMA，则 $M_i^{\mathrm{nma}}$ 视为 0，退化为 $w_i^{\mathrm{eff}}=w_i^{\mathrm{pocket}}$。
可选消融：使用 $w_i^{\mathrm{eff}}=\max(w_i^{\mathrm{pocket}},\lambda M_i^{\mathrm{nma}})$ 或引入时间衰减（主线固定 $\lambda=1$，无时间衰减）。
...
$$
若未启用 NMA，则门控输入中的 $M_i^{\mathrm{nma}}$ 置 0。

**实现口径（主线不做硬冻结）**：不对残基集合做硬阈值 mask/冻结。网络对所有残基输出 $(\xi_i^{\mathrm{raw}},\dot\chi_i^{\mathrm{raw}})$，再由连续门控 $g_i(t)$ 缩放；同时通过 $\mathcal L_{\mathrm{bg}}$ 对低 $w^{\mathrm{eff}}$ 区域显式惩罚残余速度。该“软子空间约束”避免硬 mask 的边界伪影，并允许铰链/结构域发生协同的刚体运动。
...
## 6.3 肽键几何护栏（链连接一致性，推荐默认开启）

仅靠 FAPE/clash/smoothness 仍可能出现“端点合理但中间肽键几何轻微漂移”的情况。为保证链连接一致性，在路径上加入最小的肽键几何护栏。

对每个时间点 $t_j$，令 $\mathbf x_{N,i}(t_j),\mathbf x_{C\alpha,i}(t_j),\mathbf x_{C,i}(t_j)$ 为残基 $i$ 的 backbone 原子坐标（由 $\mathbf X(t_j)$ 取对应 atom14），仅对连续残基对 $(i,i{+}1)$ 计算：

**键长约束（必需）**：
$$
d^{CN}_i(t_j)=\|\mathbf x_{C,i}(t_j)-\mathbf x_{N,i+1}(t_j)\|_2,\qquad
\mathcal L_{\mathrm{pep\_bond}}=\sum_{j=0}^{T}\sum_{i}\big(d^{CN}_i(t_j)-d^{CN}_0\big)^2.
$$

**键角约束（可选但推荐）**：
$$
\begin{aligned}
\theta^{C\alpha C N}_i(t_j)&=\angle\big(\mathbf x_{C\alpha,i}(t_j),\mathbf x_{C,i}(t_j),\mathbf x_{N,i+1}(t_j)\big),\\
\theta^{C N C\alpha}_i(t_j)&=\angle\big(\mathbf x_{C,i}(t_j),\mathbf x_{N,i+1}(t_j),\mathbf x_{C\alpha,i+1}(t_j)\big),
\end{aligned}
$$
$$
\mathcal L_{\mathrm{pep\_angle}}=\sum_{j=0}^{T}\sum_{i}\Big(
\big(\theta^{C\alpha C N}_i(t_j)-\theta^{C\alpha C N}_0\big)^2+
\big(\theta^{C N C\alpha}_i(t_j)-\theta^{C N C\alpha}_0\big)^2
\Big).
$$

总的肽键护栏：
$$
\mathcal L_{\mathrm{pep}}=\mathcal L_{\mathrm{pep\_bond}}+\lambda_{\mathrm{ang}}\mathcal L_{\mathrm{pep\_angle}},
$$
其中 $(d^{CN}_0,\theta_0)$ 为标准肽键几何常数（可直接取 Stage‑1/模板中的理想值），$\lambda_{\mathrm{ang}}$ 为固定超参。

---

## 6.4 口袋接触软单调性（方向性约束，路径级）
...
## 6.5 Stage‑1 holo prior 对齐（路径后半段硬约束）
...
Stage‑2 的大尺度变构能力来自 $M_i^{\mathrm{nma}}$。该特征在训练与推理中都由 apo 结构预计算，作为**可选插件**使用（默认关闭；在大变构子集或消融中启用）。
...
+\lambda_{\mathrm{clash}}\mathcal L_{\mathrm{clash}}
+\lambda_{\mathrm{pep}}\mathcal L_{\mathrm{pep}}
...
1. 从 apo/holo 构造 $x_0,x_1$；从 apo 构造 pocket 权重 $w^{\mathrm{pocket}}$；若启用 NMA，预加载 $M^{\mathrm{nma}}$；
2. 运行 Stage‑1 得到 $\tilde x_1$（消融时可跳过并令 $\log\delta F,\delta\chi=0$）；
3. 采样 $t\sim\mathcal U(0,1)$，构造 $X_t^{\mathrm{ref}},u_t^{\mathrm{ref}}$；
...
6. 计算路径级 $\mathcal L_{\mathrm{smooth}},\mathcal L_{\mathrm{clash}},\mathcal L_{\mathrm{pep}},\mathcal L_{\mathrm{contact}},\mathcal L_{\mathrm{prior}}$；
...
1. 构造 $x_0$；
2. 计算 $w^{\mathrm{pocket}}$（apo+ligand），若启用 NMA 读取 $M^{\mathrm{nma}}$；
3. 运行 Stage‑1 得到 $\tilde x_1$（消融时可跳过并令 $\log\delta F,\delta\chi=0$）；
...
5. **pose 噪声敏感性曲线（必做）**：对 ligand bound pose 注入不同 RMSD 的扰动（或使用 docked pose），报告终点与路径指标（clash/contact/smooth/end）随 pose 误差的退化曲线，明确“known pose 主线”的适用边界与鲁棒性。
