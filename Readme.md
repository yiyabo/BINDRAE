...
**Stage‑1** 学习一个高保真、几何一致的 **holo 先验解码器**，以冻结的蛋白语言模型（ESM）为语义底座，并在 SE(3) 等变结构模块中融合配体条件，输出 per‑residue backbone 刚体帧与侧链 χ 扭转角，通过前向运动学（FK）重建全原子结构；
...
使用冻结蛋白语言模型 (E(\cdot))：
[
e_i = E(S)*i\in\mathbb R^{d*{\mathrm{esm}}}.
]
...
Stage‑1 使用 SE(3) 等变的主干 (\mathrm{Trunk}*\psi) 将 ((s_i,b*{ij},l_i,F^{\mathrm{apo}})) 映射到残基隐状态 (h_i) 与预测帧 (\tilde F_i)。抽象表示为：
[
...
\left|
\pi(\tilde F_i,\tilde{\mathbf x}*{j,a})-\pi(F_i^{\ast},\mathbf x^{\mathrm{holo}}*{j,a})
\right|_2.
]
...
[
\mathcal L_{\chi}=\sum_{i,k} m_{i,k}\ d_{S^1}(\tilde\chi_{i,k},\chi^{\mathrm{holo}}*{i,k})^2,
\quad
d*{S^1}(a,b)=\mathrm{wrap}(a-b).
]
...
定义口袋权重
[
w^{\mathrm{pocket}}*i=\sigma!\left(\frac{d_0-d_i}{\tau}\right)\in(0,1),
]
其中 (d_0) 为接触阈值（例如 6Å），(\tau) 为平滑尺度。Stage‑1 的残基相关损失采用加权：
[
\mathcal L*{\mathrm{Stage1}}=
\mathcal L_{\mathrm{FAPE}}+
\lambda_\chi\sum_i w^{\mathrm{pocket}}*i,\mathcal L*{\chi,i}
...
\left(
|\xi_{\Theta,i}(X_t^{\mathrm{ref}},t)-u_{F,i}^{\mathrm{ref}}(t)|*2^2
+\sum*{k=1}^4 m_{i,k}\ |\dot\chi_{\Theta,i,k}(X_t^{\mathrm{ref}},t)-u_{\chi,i,k}^{\mathrm{ref}}(t)|^2
\right)
\right].
]
...
|\log(\Delta F_i^{(j)})|*2^2
+
\sum*{k}m_{i,k}|\Delta\chi_{i,k}^{(j)}|^2
\right).
]
...
\left(
|\log(F_i(t_j)^{-1}\tilde F_{1,i})|*2^2
+
\sum_k m*{i,k}, d_{S^1}(\chi_{i,k}(t_j),\tilde\chi_{1,i,k})^2
\right).
]
...
\left(
|\xi_i(x,t)|*2^2
+\sum_k m*{i,k},|\dot\chi_{i,k}(x,t)|^2
\right).
]
...
\sum_i |\log(F_{\Theta,i}(1)^{-1}F_{1,i})|*2^2
+
\sum*{i,k}m_{i,k} d_{S^1}(\chi_{\Theta,i,k}(1),\chi_{1,i,k})^2
+
...
5. 从 (x_0) 积分得到完整路径 ({x_\Theta(t_j)}*{j=0}^T)，并解码 (\mathbf X(t_j)=\mathrm{FK}(x*\Theta(t_j)))；
...
# 10. 以 Nature 标准必须写清的“实验与可复现要点”（作为你们接下来落地清单）

虽然你此刻要求的是“写法”，但 Nature 级别评审会把这部分当作**是否可信**的核心。这里我把必须出现的要点写成“硬条款”，不提供任何“可选兼容”。
