...
class Stage1Trainer:
    ...
    def compute_loss(self, outputs: Dict, batch, step: int) -> Dict[str, torch.Tensor]:
        ...
        # 2. Cα距离损失（简化，用Ca坐标）
        # TODO: 完整版需要FK还原全原子后计算
        loss_dist = distance_loss(batch.Ca, batch.Ca, w_res_warmed) * 0  # 占位
        
        # 3. Clash惩罚（需要全原子坐标）
        # TODO: FK还原后计算
        loss_clash = torch.tensor(0.0, device=self.device)  # 占位
        
        # 4. FAPE损失（需要预测的frames）
        # TODO: 使用rigids_final计算
        loss_fape = torch.tensor(0.0, device=self.device)  # 占位
