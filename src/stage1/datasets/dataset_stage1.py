...
def compute_pocket_weights(ca_coords: np.ndarray,
    ...
    # sigmoid
    exponent = np.clip((d_min - d0) / max(tau, 1e-6), -50.0, 50.0)
    w_res = 1.0 / (1.0 + np.exp(exponent))
...
class ApoHoloTripletDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 sample_metadata_file: Optional[str] = None,
        ...
        Args:
            ...
            sample_metadata_file: Optional JSON file mapping sample_id -> metadata
                                  (for example, residue counts produced by
                                  scripts/validate_triplets_data.py).
        ...
        self._sample_metadata_cache: Optional[Dict[str, Dict]] = None
        ...
        if valid_samples_file is not None:
            valid_path = self._resolve_optional_path(valid_samples_file)
    ...
    def _resolve_optional_path(self, path_str: Optional[str]) -> Optional[Path]:
        if path_str is None:
            return None
        path = Path(path_str)
    ...
    def load_sample_metadata(self, metadata_file: Optional[str] = None) -> Dict[str, Dict]:
        metadata_path = self._resolve_optional_path(metadata_file or self.sample_metadata_file)
        if metadata_path is None:
            raise ValueError("sample_metadata_file is required to load sample metadata")

        if self._sample_metadata_cache is not None:
            return self._sample_metadata_cache

        if not metadata_path.exists():
            raise FileNotFoundError(
                f"sample metadata file not found: {metadata_path}. "
                "Generate it with scripts/validate_triplets_data.py."
            )
        ...
        if not isinstance(metadata, dict):
            raise ValueError(
                f"Unsupported metadata format in {metadata_path}; expected JSON object"
            )

        self._sample_metadata_cache = metadata
        return metadata

    def get_sample_residue_counts(self, metadata_file: Optional[str] = None) -> List[int]:
        metadata_path = metadata_file or self.sample_metadata_file
        if metadata_path is None:
            counts = []
            for sample in self.samples:
                n_res = sample.get('n_residues')
                if n_res is None:
                    raise ValueError(
                        "Sample residue metadata is unavailable; provide sample_metadata_file "
                        "generated by scripts/validate_triplets_data.py."
                    )
                counts.append(int(n_res))
            return counts

        metadata = self.load_sample_metadata(metadata_path)
        counts: List[int] = []
        missing = []
        for sample in self.samples:
            ...
            if sample_meta is None:
                ...
                continue
            if 'n_residues' not in sample_meta:
                raise KeyError(
                    f"sample metadata for {sample_id} is missing 'n_residues'"
                )
            counts.append(int(sample_meta['n_residues']))

        if missing:
            preview = ', '.join(missing[:5])
            suffix = '...' if len(missing) > 5 else ''
            raise KeyError(
                f"sample metadata missing for {len(missing)} samples: {preview}{suffix}"
            )
